  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="办事处名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入办事处名称" clearable @keyup.enter="handleQuery" />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="officeList">
      ...
      <el-table-column label="办事处名称" align="center" prop="name" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="办事处名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入办事处名称" />
        </el-form-item>
...
import { listOffice, createOffice, updateOffice, bindStreets, getSelectableAreaTree } from '@/api/org/office'
...
function getList() {
  loading.value = true
  listOffice(queryParams.value).then(res => {
    const page = res.data || res
    officeList.value = page.records || []
...
function resetQuery() {
  queryParams.value = { page: 1, size: 10, name: '', state: null }
  handleQuery()
}
...
function submitBindStreets() {
  bindStreets({ orgId: currentOfficeId.value, streetIds: selectedTreeKeys.value }).then(() => {
...
function submitForm() {
  proxy.$refs.formRef.validate(valid => {
    if (valid) {
      if (form.value.id) {
        updateOffice(form.value).then(() => {
          proxy.$modal.msgSuccess('修改成功')
          open.value = false
          getList()
        })
      } else {
        createOffice(form.value).then(() => {
